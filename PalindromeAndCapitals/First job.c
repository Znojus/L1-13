...
int main(int argc, char *argv[])
{
	...
	for(i=0;i<10;i++){
		...
		else if (codigo ==3) // is a palindrome
		{
			int i=0;
			int start=0;
			int end=strlen(nombre)-1;
			int found=0;
			
			while (start<end){
				if (tolower(nombre[start]) != tolower(nombre[end])){
					found++;
				}
				start++;
				end--;
			...
			if(found>0)
				sprintf(respuesta, "is not");
			else
				sprintf(respuesta, "is");
		}
		else if (codigo ==4) // to capitals
		{
